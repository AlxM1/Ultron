...
export async function buildProfile(creatorName) {
    // Get creator
    const creatorRes = await pool.query('SELECT id, name FROM creators WHERE LOWER(name) = LOWER($1)', [creatorName]);
    ...
    const transcriptRes = await pool.query(`SELECT t.text FROM transcripts t
     JOIN content c ON c.id = t.content_id
     WHERE c.creator_id = ANY($1) AND t.text IS NOT NULL AND LENGTH(t.text) > 100`, [allCreatorIds]);
    if (transcriptRes.rows.length === 0)
        return null;
    const transcripts = transcriptRes.rows.map((r) => r.text);
...
export async function getProfile(creatorName) {
    const res = await pool.query('SELECT profile_json FROM personas WHERE LOWER(creator_name) = LOWER($1)', [creatorName]);
    if (res.rows.length === 0)
        return null;
    return res.rows[0].profile_json;
}
