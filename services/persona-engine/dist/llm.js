...
function buildSystemPrompt(profile, relevantQuotes) {
...
function findRelevantQuotes(profile, question) {
...
function buildFallbackResponse(profile, question, relevantQuotes) {
...
export async function queryPersona(profile, question) {
...
function buildConsensus(profiles, question) {
    const questionWords = question.toLowerCase().split(/\s+/).filter(w => w.length > 3);
    // Find overlapping topics
    const topicCounts = new Map();
    for (const p of profiles) {
        for (const t of p.top_topics.slice(0, 5)) {
            if (!topicCounts.has(t.topic))
                topicCounts.set(t.topic, []);
