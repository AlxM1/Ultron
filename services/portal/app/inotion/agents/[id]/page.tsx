  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex items-center justify-center">
        <div className="text-sm text-zinc-400 dark:text-zinc-600 animate-pulse">Loading agent...</div>
      </div>
    );
  }
  ...
  if (!agent) {
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex flex-col items-center justify-center gap-4">
        <p className="text-sm text-zinc-600 dark:text-zinc-400">Agent not found: {id}</p>
        <Link href="/inotion/agents" className="text-xs text-amber-600 dark:text-amber-400 hover:underline transition-colors duration-200">Back to agents</Link>
      </div>
    );
  }
      ...
      <header className="sticky top-0 z-30 bg-white/80 dark:bg-zinc-950/80 backdrop-blur border-b border-zinc-200 dark:border-zinc-800">
        <div className="max-w-screen-xl mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/inotion/agents" className="flex items-center gap-1.5 text-xs text-zinc-400 dark:text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 transition-colors duration-200">
              <ArrowLeft size={12} /> Agents
            </Link>
            <span className="text-zinc-200 dark:text-zinc-700">/</span>
            <span className="text-sm font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">{agent.name}</span>
          </div>
          <ThemeToggle />
        </div>
      </header>
      ...
      <main className="max-w-screen-xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">{agent.name}</h1>
            {agent.role && <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">{agent.role}</p>}
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-[10px] text-zinc-400 dark:text-zinc-600 font-mono">since {agent.activeSince ?? "—"}</span>
            </div>
          </div>
        </div>
        ...
        {agent.description && (
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm dark:shadow-none">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-3">Description</h2>
        ...
        {(agent.errorCount7d ?? 0) > 0 && (
          <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900 rounded-2xl p-6 flex items-center gap-3 shadow-sm dark:shadow-none">
            <AlertTriangle size={16} className="text-red-500 dark:text-red-400" />
            <span className="text-sm text-red-700 dark:text-red-400 font-medium">{agent.errorCount7d} errors in the last 7 days</span>
          </div>
        )}
      </main>
    </div>
...
function StatBox({ icon: Icon, label, value, sub }: { icon: React.ElementType; label: string; value: string; sub?: string }) {
  return (
    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm dark:shadow-none hover:border-amber-500/30 transition-all duration-200">
      ...
      <div className="text-sm font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">{value}</div>
      {sub && <div className="text-[10px] font-mono text-zinc-400 dark:text-zinc-600 mt-0.5">{sub}</div>}
    </div>
  );
}
