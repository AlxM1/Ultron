    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      <header className="sticky top-0 z-30 bg-zinc-950/90 backdrop-blur border-b border-zinc-800">
        <div className="max-w-screen-2xl mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-sm font-semibold text-amber-500">Content Opportunities</h1>
          </div>
          <ThemeToggle />
        </div>
      </header>
      ...
      <main className="max-w-screen-2xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-1.5 text-xs text-zinc-300 focus:outline-none focus:border-amber-600"
            >
              <option value="">All Creators</option>
              {creatorList.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          <div className="text-xs text-zinc-500">{filtered.length} themes found</div>
        </div>
        ...
        {filtered.length > 0 ? (
          <div className="space-y-3">
            {filtered.map((group, i) => (
              <div key={i} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden hover:border-zinc-700 transition-colors">
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="text-lg font-bold text-amber-500 tabular-nums w-16">{group.count.toLocaleString()}</div>
                    <div>
                      <div className="text-sm text-zinc-200 font-medium">{group.theme}</div>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex gap-1">
                      {group.creators.slice(0, 3).map(c => (
                        <span key={c} className="inline-block px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-400 text-[10px] border border-zinc-700">
                          {c}
                        </span>
                      ))}
                      {group.creators.length > 3 && (
                        <span className="inline-block px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-500 text-[10px] border border-zinc-700">
                          +{group.creators.length - 3}
                        </span>
                      )}
                    </div>
                ...
                {expandedIdx === i && (
                  <div className="border-t border-zinc-800 px-5 py-4 space-y-3">
                    ...
                    <div className="flex items-center gap-2 pt-2">
                      <div className="text-[10px] text-zinc-500 uppercase tracking-wider font-medium">Audiences</div>
                      <div className="flex flex-wrap gap-1">
                        {group.creators.map(c => (
                          <span key={c} className="inline-block px-2 py-0.5 rounded bg-cyan-500/10 text-cyan-400 text-[10px] border border-cyan-500/20">
                            {c}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="pt-2">
                      <button className="text-xs px-3 py-1.5 rounded-lg bg-amber-500/10 text-amber-500 border border-amber-500/20 hover:bg-amber-500/20 transition-colors flex items-center gap-1.5">
                        <FileText size={12} />
                        Use for Script
                      </button>
                    </div>
          ...
          <div className="text-center py-16 text-zinc-500 text-sm">No data available</div>
        )}
      </main>
    </div>
  );
