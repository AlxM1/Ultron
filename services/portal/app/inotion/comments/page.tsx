      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <section>
          <SectionHeader
            ...
            action={<Link href="/inotion/comments/opportunities" className="text-xs text-cyan-500 hover:text-cyan-400 transition-colors">View All</Link>}
          />
      ...
      {filtered.length > 0 ? (
        <div className="space-y-2">
          {filtered.map((topic, i) => (
            <div key={topic.topic} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden hover:border-zinc-700 transition-colors">
              <button onClick={() => setExpandedIdx(expandedIdx === i ? null : i)} className="w-full px-5 py-4 flex items-center justify-between text-left">
                <div className="flex items-center gap-4">
                  <div className={`p-2 rounded-lg ${topic.direction === "rising" ? "bg-green-500/10" : "bg-red-500/10"}`}>
                    {topic.direction === "rising" ? <TrendingUp size={16} className="text-green-400" /> : <TrendingDown size={16} className="text-red-400" />}
                  </div>
                  <div>
                    <div className="text-sm text-zinc-200 font-medium">{topic.topic}</div>
                    <div className="text-[10px] text-zinc-500 mt-0.5 tabular-nums">{topic.mentions.toLocaleString()} mentions</div>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className={`text-sm font-bold tabular-nums ${topic.direction === "rising" ? "text-green-400" : "text-red-400"}`}>
...
function KeywordsTab() {
  ...
  if (keywords.length === 0) {
    return <div className="flex items-center justify-center h-[60vh] text-zinc-500 text-sm">No keyword data available</div>;
  }
    ...
    <div className="space-y-6">
      ...
      <section className="bg-zinc-900/50 border border-zinc-800/50 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-zinc-300 flex items-center gap-2"><Tag size={14} className="text-amber-500" /> Keyword Bubble Map</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section className="bg-zinc-900/50 border border-zinc-800/50 rounded-xl p-6">
          <h2 className="text-sm font-semibold text-zinc-300 mb-3">Keyword Trends</h2>
          <div className="flex gap-2 mb-3">
            <div className="relative flex-1">
              <Search size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-zinc-600" />
              <input value={searchInput} onChange={e => setSearchInput(e.target.value)} onKeyDown={e => e.key === "Enter" && addFromSearch()} placeholder="Add keyword..." className="w-full pl-7 pr-3 py-1.5 text-xs bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-amber-500/50" />
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            <div className="inline-grid gap-px" style={{ gridTemplateColumns: `120px repeat(${heatmapKeywords.length}, 56px)` }}>
              ...
              {heatmapKeywords.map(kw => (
                <div key={kw} className="text-[9px] text-zinc-500 text-center py-1 -rotate-45 origin-bottom-left h-12 flex items-end justify-center">{kw}</div>
              ))}
                  ...
                  {heatmapKeywords.map((kw, ci) => {
                    ...
                    return (
                      <div key={`${creator}-${kw}`} className="w-14 h-8 rounded-sm flex items-center justify-center text-[9px] cursor-default relative group" style={{ backgroundColor: `rgba(245, 158, 11, ${intensity * 0.8 + 0.05})` }}>
                        ...
                        <div className="absolute -top-8 left-1/2 -translate-x-1/2 hidden group-hover:block bg-zinc-800 border border-zinc-700 text-zinc-200 text-[10px] px-2 py-1 rounded whitespace-nowrap z-10">{creator} / {kw}: {val}</div>
                      </div>
                    );
                  })}
                </>
    ...
    <div className="space-y-8">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 space-y-4">
        <div>
          ...
          {loadingCreators ? (
            <div className="flex items-center gap-2 text-zinc-500 text-xs py-2"><Loader2 size={14} className="animate-spin" /> Loading creators...</div>
          ) : creatorOptions.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {creatorOptions.map(name => (
                <button key={name} onClick={() => toggleCreator(name)}
                  className={`text-xs px-3 py-1.5 rounded-lg border transition-colors ${selected.includes(name) ? "bg-amber-500/15 border-amber-500/40 text-amber-400" : "bg-zinc-800 border-zinc-700 text-zinc-400 hover:border-zinc-600"}`}
                >{name}</button>
              ))}
            </div>
          ) : (
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1 max-w-sm">
            ...
            <input type="text" value={topic} onChange={e => setTopic(e.target.value)} placeholder="Topic keyword (optional)" className="w-full bg-zinc-800 border border-zinc-700 rounded-lg pl-9 pr-3 py-2 text-xs text-zinc-300 placeholder-zinc-600 focus:outline-none focus:border-amber-600" />
          </div>
          <button onClick={runComparison} disabled={selected.length < 2 || loading} className="text-xs px-4 py-2 rounded-lg bg-amber-500 text-zinc-950 font-medium hover:bg-amber-400 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
            {loading ? "Analyzing..." : "Compare"}
          </button>
        </div>
        {selected.length > 0 && selected.length < 2 && <div className="text-[10px] text-zinc-500">Select at least 2 creators to compare</div>}
      </div>
      ...
      {hasSearched && results.length > 0 && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
            {results.map((r, i) => (
              <div key={r.creator} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4" style={{ borderTopColor: COMPARE_COLORS[i], borderTopWidth: 2 }}>
      ...
      {!hasSearched && (
        <div className="text-center py-16 text-zinc-600 text-sm">Select creators and click Compare to see side-by-side analysis</div>
      )}
    </div>
  );
}
