...
function drawCoverPage(doc, audit, date, w) {
    // Dark background
...
function drawExecutiveSummary(doc, audit, w) {
    doc.rect(0, 0, 595.28, 841.89).fill(COLORS.bg);
    doc.fontSize(24).fillColor(COLORS.amber).text('Executive Summary', 50, 50);
    doc.rect(50, 82, 80, 2).fill(COLORS.amberDark);
    // Overall assessment
    doc.fontSize(12).fillColor(COLORS.text).text(`Your site scored ${audit.overall_score}/100, rated "${scoreLabel(audit.overall_score)}". ` +
        `This report analyzes five key dimensions of Generative Engine Optimization to determine ` +
        `how well your content performs in AI-powered search environments.`, 50, 100, { width: w, lineGap: 4 });
...
function drawDimensionPage(doc, key, dim, w) {
    doc.rect(0, 0, 595.28, 841.89).fill(COLORS.bg);
    const meta = DIMENSION_LABELS[key];
    // Header
...
function drawCtaPage(doc, audit, w) {
    doc.rect(0, 0, 595.28, 841.89).fill(COLORS.bg);
    ...
    doc.fontSize(12).fillColor(COLORS.textMuted).text('Get a comprehensive GEO optimization strategy tailored to your business.', 100, y + 50, { align: 'center', width: w - 100 });
    doc.fontSize(14).fillColor(COLORS.amber).text('seoh.ca', 80, y + 75, { align: 'center', width: w - 60 });
}
function getRecommendationsForDimension(key, dim) {
    const recs = {
        ai_citability: [
