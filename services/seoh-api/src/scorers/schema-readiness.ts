...
export function scoreSchemaReadiness($: CheerioAPI): ScoreResult {
  let score = 0;
  const issues: string[] = [];
  ...
  let ogScore = 0;
  if (ogTitle) ogScore += 5; else issues.push('Missing og:title');
  if (ogDesc) ogScore += 5; else issues.push('Missing og:description');
  if (ogImage) ogScore += 5; else issues.push('Missing og:image');
  if (ogType) ogScore += 5; else issues.push('Missing og:type');
  score += ogScore;
